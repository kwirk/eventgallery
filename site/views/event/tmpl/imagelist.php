	...
	<div class="thumbnails">
		<?php foreach($this->entries as $entry) :?>
	    	...
	    	<a class="thumbnail" href="<?php echo $this->entry->getImageUrl(null, null, true); ?>"
			    ...
			    ?><?php IF ($this->folder->cartable==1&&$this->params->get('show_cart_connector', 0)==1):?><a href="<?php echo EventgalleryHelpersCartconnector::getLink($this->entry->folder, $this->entry->file); ?>" class="button-cart-connector" title="<?php echo JText::_('COM_EVENTGALLERY_CART_CONNECTOR')?>" data-folder="<?php echo $this->entry->folder ?>" data-file="<?php echo $this->entry->file; ?>"><i class="big"></i></a><?php ENDIF ?></div>
		<?php endforeach?>
		<div style="clear: both"></div>
	</div>
</div>
