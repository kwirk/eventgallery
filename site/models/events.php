...
class EventgalleryModelEvents extends JModelLegacy
{
    ...
    function getEntries($page = 1, $entriesPerPage = 10, $tags = "", $sortAttribute = 'ordering')
    {
        ...
        $query
            = 'SELECT folder.*, count(1) overallCount
        ...
        if (count($entries) > 0) {
            foreach ($entries as $rownum => $entry) {
                ...
                if (strpos($entry->folder, '@') > -1) {
                    ...
                    $album = EventgalleryHelpersImageHelper::picasaweb_ListAlbum(
                        $values[0], $values[1], $entry->picasakey
                    );
    ...
    function getCommentCount($folder)
    {
        if (!$this->_commentCount) {
            $query
                = 'select folder, count(1) commentCount
    ...
    function getFileCount()
    {
        $db = JFactory::getDBO();
        $query
            = 'SELECT count(1) from #__eventgallery_file file join #__eventgallery_folder folder
